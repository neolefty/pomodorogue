...
Both are branches of one choice offered when a level ends: **Descend** keeps `runSeed` and increments `depth`; **Start over** mints a new `Run` at depth 1 with no carry. Fixed mode is what you get by taking the second branch every time — which is exactly what the game did unconditionally before phase 8, so phase 8 did not add a mode, it added the other button. All three branches (a death also offers **Retry this dungeon**) are one `switch` in `src/pomodoro/run.ts`; there is no mode anywhere in the code, and the words *progressive* and *fixed* appear nowhere in `src/`.
...
3. ~~**How does difficulty scale with depth?**~~ **Answered 2026-08-12: depth raises a floor under the within-level difficulty, and three other knobs ramp alongside it.** All four live in `src/game/generator/ramp.ts` — `depthFloor` (`min(0.8, (d-1) * 0.08)`, applied as `floor + within * (1 - floor)` to the *raw* `posToDifficulty` value, before each call site's own scaling and clamping), plus monster count, cover count and the digger's `dugPercentage`. Every one is an identity at depth 1, and `generator.test.ts` pins two depth-1 seeds by hash so that fixed mode stays byte-identical to phase 6 rather than approximately so. **The numbers are guesses that have not been played** — a dev-only Backspace gate skip exists to make playtesting them possible at all. The weapon-stacking question was *not* settled first; see 8 below.
4. ~~**Does the run end at some depth, or go forever?**~~ **Answered 2026-08-12: forever.** Every ramp knob caps out — the counts around depth 6, difficulty around depth 11 — so an endless run gets hard and then stays hard, rather than escalating arithmetically into nonsense. A maximum depth would have been the thing needing code; indefinite needed none. Whether depth 30 is *interesting* is a playtesting question, not a design one.
...
8. **How is the deep game balanced, when a progressive player gets stronger for free?** Raised by the phase 8 decision on 2026-08-12 to carry the whole inventory uncapped. `getWeaponsDmg` sums every `dmg` in the pack, nothing is ever consumed, and ten levels of accumulation outpaces any ramp the generator can apply. Four fixes were considered and none built: carry only the best weapon and armour, cap the effect in combat, make weapons breakable, or leave it. **Bill's call was to leave it and find out how bad it actually is** — which is right, because the alternative is tuning against a problem nobody has felt yet. Two things for whoever picks this up. First, the diagnostic question is "is the deep game too hard, or is the deep player too strong?", because those look identical from one playthrough and want opposite corrections. Second, this is a *progressive-only* problem, so the fix must not touch depth 1 — which rules the combat cap out, since summing is what the original does within a level too, and makes "carry only your best" the option that satisfies the constraint by construction. Not blocking anything; it blocks *tuning*. See "Difficulty ramp" in [08-depth.md](docs/port/08-depth.md).
...
- ~~**Give the rest-of-break timer a colour of its own.**~~ Done 2026-08-12 alongside phase 8, which put a choice screen on the same tombstone and so made the collision worse. `.timer.rest` now takes the palette's green `#4d9029`, which already means "go"; amber `#cc7722` stays with `.timer.break.warn`, the one case that actually carries urgency. Neither of the two suggested fixes was taken — a second *warm* tone would have kept both readings in the same family, which is the thing that made the first one undo the second. Raised in review 2026-08-11.
...
- [x] Phase 8 — Depth — **a level ends in a choice, and the game plays two ways with no mode flag**
- [ ] Phase 8.5 — Shared seeds ← **next**

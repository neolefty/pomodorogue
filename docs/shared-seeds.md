...
- **Does a link opened months later have to show the same level?** Raised by Bill on 2026-08-16. A seed names a level *through the generator that reads it*, so a balance change silently re-points every link ever shared. **Leaning: no, and do not build for it — but the decision is deferrable, which is the reason not to make it now.** Three things settle it if it ever comes up. (1) The expensive answer is the only real one: fidelity means keeping every retired generator alive behind a version tag, which is a museum of code that must keep compiling forever, and it prices a balance change at "plus one permanent generator". (2) It is genuinely deferrable at zero cost, unlike seeding the combat stream: a link with no version parameter means "generate with whatever is current", so adding `&g=2` later leaves today's links resolving exactly as they do now, which is also the don't-care answer. Nothing has to be decided in advance for the option to stay open. (3) The intuition that *depth 1 at least* should hold still points the wrong way — depth 1 is the level a fixed-mode player sees sixteen times a day, so it is where the game most needs to be free to change (see [progression/](progression/README.md)), not where it can most afford to be pinned. Freezing it is the mistake that was undone on 2026-08-16; see invariant 1 in [design.md](design.md).

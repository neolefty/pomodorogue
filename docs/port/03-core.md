...
- `Outcome` — `'died' | 'descended'` as of phase 3, **renamed to `'died' | 'cleared'` by phase 8**. The forward-looking name turned out to look the wrong way: phase 8 made the direction a choice the player makes *after* the level, so the shrine cannot know whether it was a way down or a way out, and the outcome now names only what the shrine actually knows. Likewise the encounter fn is called `finishLevel` rather than the original's `finish-game` — that one aged well.
